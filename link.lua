...
local function new_frame(destination, source, type, data)
  local frame = {0xaa,0xaa,0xaa,0xaa,0xaa,0xaa,0xaa,0xab,table.unpack(destination)}

  for i=1,#source do
    frame[#frame+1] = source[i]
  end
  for i=1,#type do
    frame[#frame+1] = type[i]
  end
  for i=1,#data do
  ...
  frame[#frame+1] = bit32.band(bit32.rshift(crc, 24), 0xFF);
  frame[#frame+1] = bit32.band(bit32.rshift(crc, 16), 0xFF);
  frame[#frame+1] = bit32.band(bit32.rshift(crc, 8), 0xFF);
  frame[#frame+1] = bit32.band(crc, 0xFF);
...
local function parse_address(addr)
  ...
  for hextet in string.gmatch(addr, "(%x%x)") do
    table.insert(bytes, tonumber(hextet, 16))
  end
  return bytes
end
